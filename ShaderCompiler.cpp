...
IDxcBlob* ShaderCompiler::Compile(const std::wstring& filePath, const wchar_t* profile, 
	...
	logger->Log(logger->GetStream(), logger->ConvertString(std::format(L"Begin Compile, path:{}, profile:{}\n", filePath, profile)));
	// hlslファイルを読む
	...
	assert(SUCCEEDED(hr));
	// 成功したログを出す
	logger->Log(logger->GetStream(), logger->ConvertString(std::format(L"Compile Succeeded, path:{}, profile:{}\n", filePath, profile)));
	// もう使わないリソースを解放
