...
void FireBall::Shoot(Vector3 pos, Vector3 dir,std::vector<std::unique_ptr<Bullet>>& bullets,GameContext* context) {
	if (shootCoolTimer_ <= 0) {
		...
		shootCoolTimer_ = status_.shootCoolTime;
	}
}

void FireBall::Update() {
	if (status_.shootCoolTime > 0) {
		shootCoolTimer_--;
	}
}
