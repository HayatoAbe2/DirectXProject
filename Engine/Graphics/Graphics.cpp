...
void Graphics::Initialize(int32_t clientWidth, int32_t clientHeight, HWND hwnd, Logger* logger) {
	...
	// Shaderをコンパイルする
	vertexShaderBlob_ = shaderCompiler_->Compile(L"Resources/shaders/Object3d.VS.hlsl",
		L"vs_6_0", logger_);
	assert(vertexShaderBlob_ != nullptr);

	pixelShaderBlob_ = shaderCompiler_->Compile(L"Resources/shaders/Object3d.PS.hlsl",
		L"ps_6_0", logger_);
...
void Graphics::CreatePipelineState() {
	...
	gridVSBlob_ = shaderCompiler_->Compile(L"Resources/shaders/Grid.VS.hlsl",
		L"vs_6_0", logger_);
	assert(gridVSBlob_ != nullptr);

	gridPSBlob_ = shaderCompiler_->Compile(L"Resources/shaders/Grid.PS.hlsl",
		L"ps_6_0", logger_);
